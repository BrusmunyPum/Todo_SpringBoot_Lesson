...
public class TaskMapper {
    public TaskResponse toResponse(Task task) {
        ...
        return new TaskResponse(
                task.getId(), task.getTitle(), task.isCompleted(), task.getPriority(),
                task.getDueDate(), userId, username, task.getCreatedAt(), task.getUpdatedAt()
        );
    }
    ...
    public TaskPageResponse toPageResponse(Page<Task> taskPage) {
        ...
        return new TaskPageResponse(
                content, taskPage.getNumber(), taskPage.getSize(),
